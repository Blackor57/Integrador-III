...
const API_PEDIDOS_URL = `${BASE_URL}/pedido`;
const API_DETALLES_URL = `${BASE_URL}/detalle`;
...
const DashboardService = {
    ...
    async fetchDatosDashboard() {
        ...
        const ingresosHoy = pedidosHoy.reduce((acc, p) => acc + (p.total || p.precioTotal || 0), 0);
        const gastoPromedio = totalOrdenesHoy > 0 ? (ingresosHoy / totalOrdenesHoy) : 0;
        ...
        const pedidosMensualesValidos = todosLosPedidos.filter(p => {
            ...
            const estado = String(p.estadopedido || p.estadoPedido || p.estado || "").toUpperCase();
            return estado === "PAGADO" || estado === "ENTREGADO" || estado === "FINALIZADO";
        });
        const ingresosMensuales = pedidosMensualesValidos.reduce((acc, p) => acc + (p.total || p.precioTotal || 0), 0);
    ...
    async fetchPlatosTendencia() {
        ...
        detalles.forEach(d => {
            ...
            if (producto) {
                ...
                const estadoPedido = pedido ? String(pedido.estadopedido || pedido.estadoPedido || pedido.estado || "").toUpperCase() : "";

                // Agrupamos por id del producto
                const id = producto.id;
                if (!conteoPlatos[id]) {
                    conteoPlatos[id] = {
                        nombre: producto.nombreproducto || producto.nombreProducto || producto.nombre || "Plato Desconocido",
...
async function cargarPlatosTendencia() {
    try {
        ...
        const gridPlatos = document.getElementById("contenedor-platos-tendencia") ||
        ...
        if (gridPlatos) {
            ...
            gridPlatos.innerHTML = topPlatos.map((plato, index) => `
...
function actualizarGraficos(pedidos) {
    ...
    const mapeoEstados = {};
    pedidos.forEach(p => {
        const campoEstado = p.estadopedido || p.estadoPedido || p.estado || "INDEFINIDO";
    ...
    if (donutChartInstance && etiquetas.length > 0) {
        ...
        const paletaColores = ['#f5be38', '#34d399', '#60a5fa', '#c084fc', '#f87171', '#a8a29e'];
        donutChartInstance.data.datasets[0].backgroundColor = etiquetas.map((_, i) => paletaColores[i % paletaColores.length]);
        donutChartInstance.update();

        const leyenda = document.querySelector(".mt-4.space-y-2.text-xs");
        if (leyenda) {
            leyenda.innerHTML = etiquetas.map((est, i) => {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (ctxBar) {
        barChartInstance = new Chart(ctxBar, {
            ...
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, grid: { color: '#1c0e0a', drawBorder: false }, ticks: { precision: 0 } }, x: { grid: { display: false, drawBorder: false } } } }
        });
    }
